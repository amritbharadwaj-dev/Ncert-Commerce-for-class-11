...
function injectGlobalWidget() {
    if (!document.getElementById('global-widget-root')) {
        ...
        <div id="global-widget-root">
            <div id="floating-avatar-btn" onclick="handleAvatarClick(event)" class="fixed bottom-24 right-5 z-50 w-24 h-24 rounded-full bg-gradient-to-tr from-purple-600 to-indigo-500 p-1 shadow-2xl cursor-pointer hover:scale-105 transition-transform duration-200 flex items-center justify-center select-none touch-none">
                <div class="w-full h-full rounded-full bg-slate-900 overflow-hidden flex items-center justify-center border-3 border-white dark:border-slate-800">
...
// Message Bubble with Timed Disappearance (User: 1s, Admin: 3s)
...
// Telegram Polling with 3 seconds disappearance for incoming messages
                ...
                data.result.forEach(update => {
                    ...
                    if (messageObj && messageObj.text) {
                        if (String(messageObj.chat.id) === String(TG_CHAT_ID)) {
                            appendMessageBubble(`Admin: ${messageObj.text}`, 'admin', 3000); // 3 seconds disappear
                        }
                    }
                });
            }
