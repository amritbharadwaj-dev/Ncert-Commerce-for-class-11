// Global Widget & Telegram Integration Logic with Draggable Cartoon Avatar
...
function injectGlobalWidget() {
    if (!document.getElementById('global-widget-root')) {
        ...
        <div id="global-widget-root">
            <div id="floating-avatar-btn" onclick="handleAvatarClick(event)" class="fixed bottom-24 right-5 z-50 w-16 h-16 rounded-full bg-gradient-to-tr from-purple-600 to-indigo-500 p-0.5 shadow-2xl cursor-pointer hover:scale-105 transition-transform duration-200 flex items-center justify-center select-none touch-none">
                <div class="w-full h-full rounded-full bg-slate-900 overflow-hidden flex items-center justify-center border-2 border-white dark:border-slate-800">
                    <img src="https://api.dicebear.com/7.x/bottts/svg?seed=AmritBot" alt="Cartoon Avatar" class="w-full h-full object-cover">
                </div>
            </div>
                ...
                <div class="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-3xl p-5 space-y-4 shadow-2xl border-t border-slate-200 dark:border-slate-800 max-h-[85vh] flex flex-col">
                    ...
                    <div class="overflow-y-auto space-y-4 pr-1 flex-grow">
                        ...
                        <div class="flex items-center space-x-2 pt-1">
                            ...
                            <button onclick="sendTelegramMessage()" class="bg-black hover:bg-slate-800 text-white dark:bg-white dark:text-black dark:hover:bg-slate-200 px-5 py-3 rounded-xl text-xs font-bold transition shadow-md flex items-center space-x-1">
                                <span>Send</span>
...
// Track dragging vs clicking so avatar can be moved anywhere without opening drawer accidentally
...
// Send message to Telegram Bot & auto disappear in 1 second
async function sendTelegramMessage() {
    ...
    appendMessageBubble(`You: ${text}`, 'user', 1000); // 1 second disappearance for user message
    msgInput.value = '';
...
// Append message bubble with custom disappear timeout (User: 1s, Admin: 3s)
...
// Polling telegram messages with 3 seconds disappearance
                ...
                data.result.forEach(update => {
                    ...
                    if (messageObj && messageObj.text) {
                        if (String(messageObj.chat.id) === String(TG_CHAT_ID)) {
                            appendMessageBubble(`Admin: ${messageObj.text}`, 'admin', 3000); // 3 seconds disappearance for telegram incoming message
                        }
                    }
                });
            }
